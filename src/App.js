...
import { ShoppingCart, Search, X, Plus, Minus, ChevronUp, ChevronDown, Star, Truck, Shield, Award, Menu, Phone, Mail, Heart, Filter } from 'lucide-react';

const Toast = ({ message, type = 'success', onClose }) => ( <
    div className = { `fixed top-4 right-4 p-4 rounded-xl shadow-2xl z-50 animate-slide-in flex items-center space-x-3 ${type === 'success' ? 'bg-green-500 text-white' : 'bg-red-500 text-white'}` } >
    <
    span > { message } < /span>   <
    button onClick = { onClose }
    className = "text-white hover:text-gray-200" > < X size = {}
    /> </button >
    <
    /div>
);
...
const OrderConfirmation = ({ order, onClose }) => ( <
    ...
    p className = "text-gray-600 mb-4" > Merci pour votre commande de { order.totalItems }
    gabarits < /p> <
...
const AdvancedFilters = ({ filters, onFiltersChange, series }) => {
    ...
    return ( <
        ...
        {
            isOpen && ( < div className = "grid grid-cols-1 md:grid-cols-3 gap-4 animate-slide-down" >
                <
                div >
                <
                ...
                div className = "space-y-2" > {
                    series.map(s => ( <
                        ...
                        onChange = {
                            (e) => {
                                const newSeries = e.target.checked ? [...filters.series, s] :
                ...
                div className = "space-y-2" > {
                    ['S', 'M', 'L'].map(size => ( <
                        ...
                        onChange = {
                            (e) => {
                                const newSizes = e.target.checked ? [...filters.sizes, size] :
                ...
                onChange = {
                    (e) => onFiltersChange({...filters, maxPrice: parseInt(e.target.value) })
                }
                className = "w-full" /
                >
                <
...
const ProductCard = ({ product, onAddToCart, isCarousel = false, isFavorite, onToggleFavorite, onView }) => ( <
    div className = { `bg-white rounded-xl lg:rounded-2xl transition-all duration-300 hover:scale-105 ${
      isCarousel 
    ...
    <
    span className = "text-2xl lg:text-4xl font-bold text-white drop-shadow" > { product.series } < /span> <
    div className = "text-right" >
    <
    span className = "bg-white/90 backdrop-blur-sm px-3 lg:px-4 py-1 lg:py-2 rounded-full text-amber-700 text-sm lg:text-lg font-mono font-bold block shadow" > { product.model } <
    /span> <
    ...
    <
    span className = "text-white text-xs lg:text-sm font-semibold" > { product.rating } < /span> < /
    div > <
    /div> < /
    div > <
    /div>
    ...
    <
    h3 className = "text-lg lg:text-xl font-bold text-gray-800 mb-1 lg:mb-2 group-hover:text-amber-600 transition-colors" > { product.name } <
    /h3> <
    p className = "text-xs lg:text-sm text-gray-600 mb-3 lg:mb-4 leading-tight lg:leading-relaxed" > { product.description } <
    /p>
    ...
    div >
    <
    span className = "text-xl lg:text-3xl font-bold text-amber-600" > { product.basePrice }€ < /span> <
    ...
    <
    Plus size = { 14 }
    className = "mr-1 lg:mr-2 text-amber-600" / >
    Tailles:
    <
    /p> <
    div className = "grid grid-cols-3 gap-2" > {
...
const BarrelCarousel = ({ products, onAddToCart, favorites, onToggleFavorite, onView }) => {
    ...
    return ( <
        div className = "relative w-full py-8 lg:py-16 bg-gradient-to-b from-gray-50 to-white rounded-xl lg:rounded-3xl" > {!isMobile && ( <
        ...
        <
        div className = { `relative w-full ${isMobile ? 'h-[350px]' : 'h-[400px]'} flex items-center justify-center` } > {
        ...
        {!isMobile && ( <
                ...
                <
                div className = "text-amber-600 text-sm font-semibold bg-white/80 backdrop-blur-sm px-4 py-2 rounded-full shadow" > { currentIndex + 1 }
                / {products.length} < /
                div > <
                button onClick = {
                    () => handleScroll('down')
        ...
        <
        div className = "text-gray-700 font-bold" > { currentIndex + 1 }
        / {products.length} < /
        div > <
        /div>
...
const Cart = ({ cart, cartTotal, onUpdateQuantity, onRemoveFromCart, onClose, showCart, onCheckout }) => {
    ...
    return ( <
        ...
        div className = "p-4 lg:p-6 space-y-4" > {
            cart.length === 0 ? ( <
                ...
                className = "mx-auto text-gray-300 mb-4 lg:mb-6" / >
                <
                p className = "text-lg lg:text-xl font-semibold text-gray-800 mb-2" > Votre panier est vide < /p> <
                p className = "text-gray-600 text-sm lg:text-base" > Ajoutez des gabarits pour commencer < /p> < /
                div >
            ) : ( <
                >
                {
                    cart.map((item, index) => ( <
                        ...
                        <
                        h3 className = "font-bold text-gray-800 text-base lg:text-lg" > { item.name } < /h3> <
                        p className = "text-xs lg:text-sm text-gray-600" > Taille: < span className = "font-semibold" > { item.size } < /span></p >
                        <
                        /div> <
                        button onClick = {
                            () => onRemoveFromCart(item.id, item.size)
                        }
                        ...
                        <
                        /button> <
                        span className = "font-bold text-gray-800 min-w-[30px] lg:min-w-[40px] text-center text-sm lg:text-lg" > { item.quantity } < /span> <
                        button onClick = {
                            () => onUpdateQuantity(item.id, item.size, 1)
                        }
                        ...
                        /button> < /
                        div > <
                        span className = "font-bold text-amber-600 text-base lg:text-xl" > { item.quantity * cartTotal.pricePerUnit }€ < /span> < /
                        div > <
                        /div>
                    ))
                }
                ...
                span className = "font-bold" > { cartTotal.totalItems }
                pièces < /span> < /
                div > <
                div className = "flex justify-between text-gray-700 text-sm lg:text-lg" >
                <
...

                {
                    cartTotal.totalItems > 0 && ( <
                        ...
                        span > { cartTotal.totalItems }
                        /10</span >
                        <
                        /div> <
                        ProgressBar current = { cartTotal.totalItems }
                        target = { 10 }
                        /> < /
                        div >
                    )
...

                {
                    cartTotal.totalItems >= 10 && ( <
                        ...
                        <
                        p className = "text-green-700 font-bold text-sm lg:text-lg" > 🎉Remise appliquée!
                        <
                        /p> < /
                        div >
                    )
                ...
                className = "w-full bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 text-white font-bold py-3 lg:py-4 rounded-xl lg:rounded-2xl shadow-xl hover:shadow-2xl transition-all duration-300 text-base lg:text-xl" > 🚀Commander <
                /button> < /
                div > <
                />
            )
...
function App() {
    ...
    const addToCart = (product, size) => {
        ...
        if (existingItem) {
            setCart(cart.map(item =>
                item.id === product.id && item.size === size ? {...item, quantity: item.quantity + 1 } :
                item
            ));
        } else {
    ...
    return ( <
...

            {
                showOrderConfirmation && ( <
                    OrderConfirmation order = { cartTotal }
                    onClose = {
            ...
            cart.length > 0 && ( <
                span className = "absolute -top-2 -right-2 bg-red-500 text-white text-xs w-5 h-5 rounded-full flex items-center justify-center font-bold shadow-lg" > { cartTotal.totalItems } <
                /span>
            )
        } <
        /button>
    ...
    header >
        ...
        <
        h2 className = "text-lg lg:text-2xl font-bold text-gray-800" > 📂Série: < span className = "text-amber-600" > { selectedSeries } < /span> < /
    h2 > <
    ...
    series.map(s => {
        ...
        return ( <
            ...
            span className = "ml-1 lg:ml-2 text-xs opacity-80" > ({ seriesProducts.length }) < /span> < /
            button >
        );
    })
} <
...
<
main className = "max-w-7xl mx-auto px-4 lg:px-6 py-6 lg:py-10" > { /* Filtres avancés */ } <
...
div className = "mb-8 lg:mb-12 bg-gradient-to-br from-white to-amber-50 rounded-xl lg:rounded-3xl border-2 lg:border-4 border-amber-500 p-4 lg:p-8 shadow-xl" >
    <
    h2 className = "text-xl lg:text-3xl font-bold text-gray-800 mb-4 lg:mb-6 text-center" > 💰Tarifs Dégressifs < /h2> <
...
div >
    ...
    { /* Produits */ } {
        filteredProducts.length === 0 ? ( <
            ...
            p className = "text-lg lg:text-2xl font-bold text-gray-800 mb-2 lg:mb-4" > Aucun gabarit trouvé < /p> <
            p className = "text-gray-600 text-sm lg:text-base" > Essayez une autre série ou modifiez vos filtres < /p> < /
            div >
        ) : viewMode === 'carousel' ? ( <
            BarrelCarousel products = { filteredProducts }
...
footer className = "bg-gradient-to-r from-gray-900 to-gray-800 text-white mt-12 lg:mt-20" >
    ...
    div >
    <
    h3 className = "text-lg lg:text-2xl font-bold text-amber-400 mb-3 lg:mb-4" > Gabarits.fr < /h3> <
...
div > <
    ...
    <
    p className = "text-gray-400 text-sm lg:text-base" > ©2024 Gabarits.fr - Tous droits réservés <
    /p> < /
div > <
    /div> < /
footer > <
